...
## 3.4 linux系统调用分析
 ...
 ```
 (1)strace作用是打印出程序在执行过程中的调用的系统调用函数。
 (2)设置内存映像保护->映射虚拟内存页->写文件->打开文件->去除内存页映射->确定文件的可存取性->
 读文件->关闭文件->取文件状态信息->改变数据段空间的分配->运行可执行文件->对进程进行特定操作
 (3)系统调用的具体执行过程：应用程序通过系统调用接口，通过软中断进入到系统内核，再转到系统调用表，根据编号选取不同的系统调用的实现（转到CPU硬件完成）得到的结果再返回系统调用接口。
 (4)扩展说明：系统调用和函数调用的区别：

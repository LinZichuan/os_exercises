...
```
（1）初始化IDT：在IDT中设置好各种中断向量对应的中断描述符，留待CPU在产生中断后，查询对应中断服务例程的起始地址。
（2）初始化8259中断控制器
（3）初始化8253时钟外设
```
```
...
int main ()
{
    int mul_val = va_mul(1, 2, 3, 4, 5, END);
    printf ("%d\n", mul_val);
    return 0;
}
```
...
> 先实现一个可显示字符串的bootloader，要处理时钟、串口、CGA等外设；
> 再实现一个可加载ELF格式文件的bootloader，要处理硬盘的中断。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 方案一：系统调用要访问中断向量表，可以在访问中断向量表前加入一段代码纪录系统调用的次数。在这段代码里可以记录某个特定的系统调用的次数，也可以记录所有的系统调用的次数。
> 方案二：可以用strace命令加特殊的参数来显示系统调用的次数。

如何裁减lab1, 实现一个可显示字符串"THU LAB1"且依然能够正确加载ucore OS的bootloader？如果不能完成实现，请说明理由。
- [x]  

> bootloader的作用是加载ucoreOS，这段代码只有不超过510字节的大小，无法容纳其余过多的代码。而且bootloader还不能处理外设的中断，无法通过串口输出"THU LAB1"的字符串。
